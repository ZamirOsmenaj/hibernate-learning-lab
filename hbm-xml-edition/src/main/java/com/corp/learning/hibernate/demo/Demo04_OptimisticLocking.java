 ...
 * overwriting the first user's change ("lost update" problem).
 */
public class Demo04_OptimisticLocking {
    ...
    public static void run() {
        ...
        try {
            ...
            // NOTE: we catch javax.persistence.OptimisticLockException here,
            // NOT org.hibernate.StaleObjectStateException directly. Just like
            // in Demo01, SessionImpl.doFlush() routes every failure through
            // ExceptionConverterImpl, whose wrapStaleStateException() converts
            // Hibernate's native StaleObjectStateException into the JPA
            // OptimisticLockException before it ever reaches this catch block -
            // even though this whole demo never touches an EntityManager.
