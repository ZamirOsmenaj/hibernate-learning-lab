...
/**
 * DEMO 2 - collection-mapping styles beyond the plain @OneToMany you already
 * know, all via @ElementCollection (value types, no identity of their own):
 *   - Department.budgetHistory - @OrderColumn (ordered, of @Embeddable value objects)
 *   - Department.tags          - @MapKeyColumn (key/value pairs)
 *   - Employee.certifications  - no @OrderColumn (unordered, duplicates allowed - a "bag")
 */
public class Demo02_Collections {
    ...
    public static void run() {
        ...
        // ---- @OrderColumn: ordered history of @Embeddable value objects ----
        ...
        // ---- @MapKeyColumn: free-form key/value tags ----
        ...
        // A base Employee (not Manager/Developer) with an unordered @ElementCollection of certifications
        ...
        dana.getCertifications().add("GDPR Fundamentals"); // duplicates ARE allowed here, unlike a Set
        session.persist(dana); // persist(), not save() - see Demo01 for why, applies uniformly here

        tx.commit();
        session.close();

        System.out.println(">> Saved list/map/bag-style collections. Re-loading in a fresh session to prove"
        ...
        System.out.println("   budgetHistory (ORDER preserved via the 'position' column, @OrderColumn):");
        int index = 0;
        for (BudgetEntry entry : reloadedDept.getBudgetHistory()) {
            System.out.println("     [" + (index++) + "] " + entry);
        }

        System.out.println("   tags (@ElementCollection + @MapKeyColumn):");
        ...
        System.out.println("   certifications, unordered @ElementCollection (size=" + certs.size()
