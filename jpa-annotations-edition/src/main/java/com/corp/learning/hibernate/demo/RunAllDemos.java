...
public class RunAllDemos {
    ...
    public static void main(String[] args) {
        try {
            banner("DEMO 1 / 6 - Basic CRUD, cascade={PERSIST,MERGE}, and the parent-delete FK guard");
            Demo01_BasicCrudAndCascade.run();

            banner("DEMO 2 / 6 - Collection mappings: @OrderColumn, @MapKeyColumn, unordered @ElementCollection");
            Demo02_Collections.run();

            banner("DEMO 3 / 6 - Inheritance: SINGLE_TABLE / JOINED / TABLE_PER_CLASS");
            Demo03_Inheritance.run();

            banner("DEMO 4 / 6 - Optimistic locking with @Version");
            Demo04_OptimisticLocking.run();

            banner("DEMO 5 / 6 - Fetching strategies: @Fetch(JOIN) / default+@BatchSize / @Fetch(SUBSELECT)");
            Demo05_FetchingStrategies.run();

            banner("DEMO 6 / 6 - mappedBy (\"inverse\") vs the owning side of an association");
